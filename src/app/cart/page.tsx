...
export default function CartPage() {
  ...
  const isUnderMinimum = total < 500;

  const handleCheckout = (e: React.FormEvent) => {
    e.preventDefault();
    if (cart.length === 0) return alert('Your cart is empty');
    if (isUnderMinimum) {
      return alert('Minimum order amount is Rs. 500. Please add more items to proceed.');
    }
            ...
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24 border border-gray-100">
              ...
              <form onSubmit={handleCheckout} className="space-y-4">
                ...
                <div className="pt-4 border-t mt-6">
                  ...
                  {/* Minimum Order Warning Card */}
                  {isUnderMinimum && (
                    <div className="mb-4 bg-red-50 text-red-700 p-3.5 rounded-xl text-xs border border-red-100 flex items-start gap-2">
                      <span className="text-sm">⚠️</span>
                      <div>
                        <strong>Order Value Under Minimum:</strong> Your current order total (Rs. {total}) is below the required <strong>Rs. 500 minimum</strong>. Please add more items to check out.
                      </div>
                    </div>
                  )}
                  ...
                  <button
                    ...
                    className={`w-full py-3.5 px-4 rounded-xl font-bold flex justify-center items-center gap-2 transition-all shadow-md ${
                      isUnderMinimum 
                        ? 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200' 
                        : 'bg-[#25D366] hover:bg-[#128C7E] text-white active:scale-95'
                    }`}
                  >
